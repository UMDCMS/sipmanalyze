...
def make_ratio_figure():
    ...
    The top and bottom axes objects with share the same x axes, with the axes
    tile and tick labels already tweaked for the standard ratio plots format.
    The top axes object will be square, and the bottom axes object will have a
    1:2 ratio.
    """
    matplot.style.use(mplhep.style.CMS)
    fig = matplot.pyplot.figure(constrained_layout=True)
    spec = fig.add_gridspec(ncols=1, nrows=2, width_ratios=[1], height_ratios=[3, 1])
...
def _get_norm1d(prange, data=None) -> float:
    ...
    For a binned data set with regular bin width, this normalization factor
    will be given as: s * w, where s is the entry count of the data within the
    specified plot range, and w with the bin width.

    For data with variable binning, the extra bin-width factor will not be
    added, instead the histogram should be modified such that it plots the
    effective density of each bin entry by divide each bin entry by the bin
...
def plot_pdf1d(
    ...
    **kwargs,
    ...
    if isinstance(data, zfit.data.Data):
        ...
        data = unbinned_to_binned(data, binning, prange=prange)
        return plot_pdf1d(
            ax=ax, pdf=pdf, data=data, scale=scale, prange=prange, **kwargs
        )
    else:
...
def _calc_density_hist(h):
    ...
    returning a temporary histogram to be used in plotting. The original
    histogram will remain unmodified.
    """
...
def plot_data1d(ax, data, **kwargs):
    ...
    instead, where each of the bin entries is divided by the bin width, that
    way, the histogram is ensured to have reliable scaling with data.
    """
    if isinstance(data, zfit.data.Data):
        binned_data = unbinned_to_binned(
            data, binning=kwargs.pop("binning"), prange=kwargs.pop("prange")
        )
...
def plot_fitratio(ax, num, den, prange=None, **kwargs):
    ...
    pnum.values()[:] = np.where(
        (x >= prange[0]) & (x <= prange[1]), pnum.values() / y, np.nan
    )
...
def number_str(
    ...
    *args,
    ...
    The *args input can be an arbitrary list of uncertainties. Uncertainties
    can either by a 2-tuple of asymmetric uncertainties, where the
    uncertainties will be indicated using the ${}^{+}_{-}$ format; or a
    singular floating point number indicating the symmetric uncertainty, where
    the uncertainty will be indicated using the ${}\pm{}$ format.
