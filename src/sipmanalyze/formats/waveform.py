...
__adc_dtype_dict__ = {2: numpy.int8, 4: numpy.int16, }
   ...
   """
    with open(filepath, 'r') as f:
      ...
      if len(settings_line) == 5:  # This is the old format with 5 inputs.
        settings = waveform_settings(timeintervals=float(settings_line[0]),
                                     triggerdelay=int(0),
                                     adc_val=float(settings_line[4]) * 256,
                                     adc_bits=int(2),
                                     invert=True)
      elif len(settings_line) == 3:  # New DRS format
        settings = waveform_settings(timeintervals=float(settings_line[2]),
                                     triggerdelay=int(0),
